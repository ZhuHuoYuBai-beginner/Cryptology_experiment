...
KEY = b"Harder ECB key!!"
RANDOM_PREFIX = b"mystery-prefix-29-bytes!!"
...
def oracle(attacker_input: bytes) -> bytes:
    return aes_ecb_encrypt(RANDOM_PREFIX + attacker_input + UNKNOWN_SUFFIX, KEY)
...
def find_prefix_alignment(block_size: int) -> tuple[int, int]:
    marker = b"B" * (block_size * 2)
    for pad_len in range(block_size):
        ciphertext = oracle(b"A" * pad_len + marker)
        blocks = [ciphertext[i : i + block_size] for i in range(0, len(ciphertext), block_size)]
        for index in range(len(blocks) - 1):
            if blocks[index] == blocks[index + 1]:
                return pad_len, index
    raise RuntimeError("Could not align unknown prefix.")


def decrypt_suffix() -> bytes:
    block_size = find_block_size()
    if not is_ecb_ciphertext(oracle(b"A" * block_size * 6), block_size):
        raise RuntimeError("Oracle is not using ECB.")

    align_pad_len, prefix_blocks = find_prefix_alignment(block_size)
    recovered = bytearray()
    total_len = len(oracle(b"A" * align_pad_len))

    for _ in range(total_len):
        fill_len = block_size - 1 - (len(recovered) % block_size)
        prefix = b"A" * (align_pad_len + fill_len)
        block_index = prefix_blocks + len(recovered) // block_size
        start = block_index * block_size
        end = start + block_size
        target_block = oracle(prefix)[start:end]

        lookup = {}
        known = b"A" * align_pad_len + b"A" * fill_len + bytes(recovered)
...
def main() -> None:
    ...
    print(f"Alignment pad length: {align_pad_len}")
    print(f"Prefix occupies blocks: {prefix_blocks}")
